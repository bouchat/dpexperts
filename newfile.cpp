...
double ldmvnorm(const vec& proposal_prior_mu, const vec& nw_mu_0, const mat& sigma)
{
    ...
    return
    -proposal_prior_mu.n_elem * HALF_LOG_2_PI
    -0.5 * val
...
{
    return
    ldmvnorm(proposal_prior_mu, nw_mu_0, proposal_prior_lambda.i()/nw_kappa_0) + ldwish(proposal_prior_lambda, nw_nu_0, nw_sigma_0);
}
...
public:
    ...
    {
      vec beta_hat = lambda * lambda_mu;
      sum_beta += beta_hat;
      sum_beta_beta_t += beta_hat * beta_hat.t();
      n += 1;
    }

    vec sample_coefficients() const
    {
...
void eliciteddp(mat y_obs, mat x,
        ...
        for (size_t i = 0; i < y.n_cols; ++i)
          {
            if (assignment[i])
              {
                vec beta = assignment[i]->sample_coefficients();
        ...
        for (size_t i = 0; i < y.n_cols; ++i)
          {
            ...
            for (size_t j = 0; j < clusters.size(); ++j)
              {
                log_acceptance_prob += clusters[j]->log_proposal_likelihood_ratio(proposal_prior_alpha, proposal_prior_beta, proposal_prior_lambda, proposal_prior_lambda * proposal_prior_mu,
